...
struct TestParameters {
  ...
  std::string liquidh5file = "bases/liquide/tensor_train_rank_10.h5";
  std::string solidh5file = "bases/solide/tensor_train_rank_10.h5";
  std::string liquidh5filekks = "bases/gradient/liquide/tensor_train_gradient_rank_10.h5";
  std::string solidh5filekks = "bases/gradient/solide/tensor_train_gradient_rank_10.h5";
...
void common_parameters(mfem::OptionsParser& args, TestParameters& p) {
  ...
  args.AddOption(&p.liquidh5filekks, "-h5liqkks", "--liquid_h5_file_kks",
                 "kks file for liquid phase");
  args.AddOption(&p.solidh5filekks, "-h5solkks", "--solid_h5_file_kks", "kks file for solid phase");
...
int main(int argc, char* argv[]) {
  ...
  std::map<std::string, std::string> hffilenameKKS = {{"C1_MO2", p.solidh5filekks},
                                                      {"LIQUID", p.liquidh5filekks}};
  std::map<std::string, std::size_t> nbreOctree = {{"C1_MO2", 12}, {"LIQUID", 12}};
  auto paramh5file = Parameter("data_filename", hffilename);
    auto paramhhffilenameKKS = Parameter("data_filenameKKS", hffilenameKKS);
  ...
  auto closure_interface = Parameter("closure_law", InterfaceClosureLaw::LinearizedKKSWithGradTT);
  ...
  auto KKS_parameters = Parameters(
      KKS_enable_specialized, KKS_secondary_phase, KKS_temperature_increment,
      KKS_composition_increment, KKS_seed, KKS_seed_radius, KKS_threshold,
      KKS_temperature_threshold, KKS_freeze_nucleation, KKS_nucleation_started, KKS_mobility,
      enable_KKS, KKS_nucleation_strategy, KKS_given_melting_temperature, paramhhffilenameKKS);
