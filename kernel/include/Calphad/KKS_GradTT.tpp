...
template <typename T>
void KKS_GradTT<T>::initialize() {
  // std::string files =
  //     "bases/gradient/solide/tensor_train_gradient_rank_10.h5";
  // std::string filel =
  //     "bases/gradient/liquide/tensor_train_gradient_rank_10.h5";

  std::string files = this->filename["C1_MO2"];
  std::string filel = this->filename["LIQUID"];

  std::vector<std::string> coord_grid = {"T", "O", "U"};

  dmu_tab_[std::make_tuple("C1_MO2", "O", "T")] = TTCores<3>();  // d(dmu_o) / dT solide
  dmu_tab_[std::make_tuple("C1_MO2", "O", "O")] = TTCores<3>();  // d(dmu_o) / dxo solide
  dmu_tab_[std::make_tuple("C1_MO2", "O", "U")] = TTCores<3>();  // d(dmu_o) / dxu solide

  dmu_tab_[std::make_tuple("C1_MO2", "U", "T")] = TTCores<3>();  // d(dmu_u) / dT solide
  dmu_tab_[std::make_tuple("C1_MO2", "U", "O")] = TTCores<3>();  // d(dmu_u) / dxo solide
  dmu_tab_[std::make_tuple("C1_MO2", "U", "U")] = TTCores<3>();  // d(dmu_u) / dxu solide

  dmu_tab_[std::make_tuple("LIQUID", "O", "T")] = TTCores<3>();  // d(dmu_o) / dT liquide
  dmu_tab_[std::make_tuple("LIQUID", "O", "O")] = TTCores<3>();  // d(dmu_o) / dxo liquide
  dmu_tab_[std::make_tuple("LIQUID", "O", "U")] = TTCores<3>();  // d(dmu_o) / dxu liquide

  dmu_tab_[std::make_tuple("LIQUID", "U", "T")] = TTCores<3>();  // d(dmu_u) / dT liquide
  dmu_tab_[std::make_tuple("LIQUID", "U", "O")] = TTCores<3>();  // d(dmu_u) / dxo liquide
  dmu_tab_[std::make_tuple("LIQUID", "U", "U")] = TTCores<3>();  // d(dmu_u) / dxu liquide

  dmu_tab_[std::make_tuple("C1_MO2", "O", "T")].import_cores_from_hdf5(files, "dmuO_grad_i",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("C1_MO2", "O", "O")].import_cores_from_hdf5(files, "dmuO_grad_j",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("C1_MO2", "O", "U")].import_cores_from_hdf5(files, "dmuO_grad_k",
                                                                       coord_grid);

  dmu_tab_[std::make_tuple("C1_MO2", "U", "T")].import_cores_from_hdf5(files, "dmuU_grad_i",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("C1_MO2", "U", "O")].import_cores_from_hdf5(files, "dmuU_grad_j",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("C1_MO2", "U", "U")].import_cores_from_hdf5(files, "dmuU_grad_k",
                                                                       coord_grid);

  dmu_tab_[std::make_tuple("LIQUID", "O", "T")].import_cores_from_hdf5(filel, "dmuO_grad_i",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("LIQUID", "O", "O")].import_cores_from_hdf5(filel, "dmuO_grad_j",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("LIQUID", "O", "U")].import_cores_from_hdf5(filel, "dmuO_grad_k",
                                                                       coord_grid);

  dmu_tab_[std::make_tuple("LIQUID", "U", "T")].import_cores_from_hdf5(filel, "dmuU_grad_i",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("LIQUID", "U", "O")].import_cores_from_hdf5(filel, "dmuU_grad_j",
                                                                       coord_grid);
  dmu_tab_[std::make_tuple("LIQUID", "U", "U")].import_cores_from_hdf5(filel, "dmuU_grad_k",
                                                                       coord_grid);

  // const std::vector<std::string> phases = {"C1_MO2", "LIQUID"};
  // const std::vector<std::string> elements = {"O", "U"};
  ...
  //         for (const auto& [var, suffix] : variables) {
  //             auto key = std::make_tuple(phase, elem, var);
  //             dmu_tab_[key] = TTCores<3>();
  //             std::string dataset_name = "dmu" + elem + "_grad_" + suffix;
  //             dmu_tab_[key].import_cores_from_hdf5(files, dataset_name, coord_grid);
  //         }
  //     }
  // }
}
 ...
 */
template <typename T>
void KKS_GradTT<T>::get_parameters(const CalphadBase<T>& CALPHAD) {
  ...
  this->filename =
      CALPHAD.params_.template get_param_value_or_default<std::map<std::string, std::string>>(
          "data_filenameKKS", {});
 ...
 */
template <typename T>
std::set<int> KKS_GradTT<T>::check_nucleation(CalphadBase<T>& CALPHAD,
                                              const std::set<int>& indices_ph_1,
                                              const T& tp_gf_ph_1) {
  ...
  switch (KKS_nucleation_strategy::from(this->KKS_nucleation_strategy_)) {
    ...
    default: {
      throw std::runtime_error(
          "KKS_GradTT<T>::check_nucleation_at_node: error in the choice of method used to compute "
 ...
 */
template <typename T>
void KKS_GradTT<T>::execute_linearization(
  ...
  if (!indices_inter.empty()) {
    ...
    for (const auto& node : indices_inter) {
      std::vector<double> tp_gf_at_node1(bar_tp_gf_ph_1.size());
      std::vector<double> tp_gf_at_node2(bar_tp_gf_ph_2.size());

      for (size_t i = 0; i < bar_tp_gf_ph_1.size(); ++i) {
        tp_gf_at_node1[i] = bar_tp_gf_ph_1[i][node];
      }
      for (size_t i = 0; i < bar_tp_gf_ph_2.size(); ++i) {
        tp_gf_at_node2[i] = bar_tp_gf_ph_2[i][node];
      }

      std::array<double, 3> point_to_interpolate1;
      std::array<double, 3> point_to_interpolate2;

      point_to_interpolate1[0] = tp_gf_at_node1[0];
      point_to_interpolate1[1] = tp_gf_at_node1[2];
      point_to_interpolate1[2] = tp_gf_at_node1[4];

      point_to_interpolate2[0] = tp_gf_at_node2[0];
      point_to_interpolate2[1] = tp_gf_at_node2[2];
      point_to_interpolate2[2] = tp_gf_at_node2[4];

      // for (const auto& el : point_to_interpolate1) {
      //   std::cout << el << ", ";
      // }
      // std::cout << std::endl;
      // for (const auto& el : point_to_interpolate2) {
      //   std::cout << el << ", ";
      // }
      // std::cout << std::endl;
      dmu_at_note_[std::make_tuple("C1_MO2", "O", "T", node)] =
          dmu_tab_[std::make_tuple("C1_MO2", "O", "T")].get_value_on_grid(point_to_interpolate1);
      dmu_at_note_[std::make_tuple("C1_MO2", "O", "O", node)] =
          dmu_tab_[std::make_tuple("C1_MO2", "O", "O")].get_value_on_grid(point_to_interpolate1);
      dmu_at_note_[std::make_tuple("C1_MO2", "O", "U", node)] =
          dmu_tab_[std::make_tuple("C1_MO2", "O", "U")].get_value_on_grid(point_to_interpolate1);

      dmu_at_note_[std::make_tuple("C1_MO2", "U", "T", node)] =
          dmu_tab_[std::make_tuple("C1_MO2", "U", "T")].get_value_on_grid(point_to_interpolate1);
      dmu_at_note_[std::make_tuple("C1_MO2", "U", "O", node)] =
          dmu_tab_[std::make_tuple("C1_MO2", "U", "O")].get_value_on_grid(point_to_interpolate1);
      dmu_at_note_[std::make_tuple("C1_MO2", "U", "U", node)] =
          dmu_tab_[std::make_tuple("C1_MO2", "U", "U")].get_value_on_grid(point_to_interpolate1);

      dmu_at_note_[std::make_tuple("LIQUID", "O", "T", node)] =
          dmu_tab_[std::make_tuple("LIQUID", "O", "T")].get_value_on_grid(point_to_interpolate2);
      dmu_at_note_[std::make_tuple("LIQUID", "O", "O", node)] =
          dmu_tab_[std::make_tuple("LIQUID", "O", "O")].get_value_on_grid(point_to_interpolate2);
      dmu_at_note_[std::make_tuple("LIQUID", "O", "U", node)] =
          dmu_tab_[std::make_tuple("LIQUID", "O", "U")].get_value_on_grid(point_to_interpolate2);

      dmu_at_note_[std::make_tuple("LIQUID", "U", "T", node)] =
          dmu_tab_[std::make_tuple("LIQUID", "U", "T")].get_value_on_grid(point_to_interpolate2);
      dmu_at_note_[std::make_tuple("LIQUID", "U", "O", node)] =
          dmu_tab_[std::make_tuple("LIQUID", "U", "O")].get_value_on_grid(point_to_interpolate2);
      dmu_at_note_[std::make_tuple("LIQUID", "U", "U", node)] =
          dmu_tab_[std::make_tuple("LIQUID", "U", "U")].get_value_on_grid(point_to_interpolate2);
 ...
 */
template <typename T>
std::unique_ptr<mfem::SparseMatrix> KKS_GradTT<T>::get_A4linearKKS(
    const std::vector<std::tuple<std::string, std::string>>& chemicalsystem,
  ...
  for (const auto& [elem, unit] : chemicalsystem) {
    if (elem != this->element_removed_from_ic_) {
      // std::cout << " get_A4linearKKS of diag   " << elem << std::endl;
      ...
      d2gd2x = dmu_at_note_[std::make_tuple(phase, elem, elem, node)];
    ...
    if (ielem != this->element_removed_from_ic_) {
      ...
      for (const auto& [jelem, unit] : chemicalsystem) {
        if (jelem != this->element_removed_from_ic_) {
          // std::cout << " get_A4linearKKS off diag   " << jelem << "  " << ielem << std::endl;
          ...
          if (ielem != jelem) {
            ...
            dmuxdy = dmu_at_note_[std::make_tuple(phase, jelem, ielem, node)];
            ...
            dmuydx = dmu_at_note_[std::make_tuple(phase, ielem, jelem, node)];
 ...
 */
template <typename T>
mfem::Vector KKS_GradTT<T>::get_m4linearKKS(
    const std::vector<std::tuple<std::string, std::string>>& chemicalsystem,
    const std::string& phase, const int node) {
  const int nb_elem = chemicalsystem.size();
 ...
 */
template <typename T>
mfem::Vector KKS_GradTT<T>::get_h4linearKKS(
    const std::vector<std::tuple<std::string, std::string>>& chemicalsystem,
    const std::string& phase, const int node) {
  const int nb_elem = chemicalsystem.size();
  ...
  for (const auto& [ielem, unit] : chemicalsystem) {
    if (ielem != this->element_removed_from_ic_) {
      ...
      // std::cout << " get_h4linearKKS   " << ielem << std::endl;
      hh(vid) = dmu_at_note_[std::make_tuple(phase, ielem, "T", node)];
 ...
 */
template <typename T>
void KKS_GradTT<T>::clear_containers() {
