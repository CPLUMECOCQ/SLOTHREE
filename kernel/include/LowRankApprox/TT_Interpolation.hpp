...
class TT_Interpolation : public CalphadBase<T> {
 private:
  ...
  std::map<std::string, std::map<std::string, TTCores<DIM>>> mapForChemicalPot;
  std::map<std::string, std::map<std::string, TTCores<DIM>>> mapForChemicalMob;
  std::map<std::string, std::map<std::string, TTCores<DIM>>> mapForChemicalEnergies;
  std::map<std::string, std::map<std::string, std::map<std::string, TTCores<DIM>>>>
...
/**
 * @brief Get the parameters associated with the TT_Interpolation object
 * @tparam T
 */
template <typename T, std::size_t DIM>
void TT_Interpolation<T, DIM>::get_parameters() {
 ...
 */
template <typename T, std::size_t DIM>
constexpr TT_Interpolation<T, DIM>::TT_Interpolation(const Parameters& params, bool is_KKS)
 ...
 */
template <typename T, std::size_t DIM>
void TT_Interpolation<T, DIM>::initialize(
  ...
  mapForChemicalPot["C1_MO2"].emplace("O", TTCores<DIM>());
  mapForChemicalPot["C1_MO2"].emplace("U", TTCores<DIM>());
  mapForChemicalPot["C1_MO2"].emplace("PU", TTCores<DIM>());
  ...
  mapForChemicalPot["LIQUID"].emplace("O", TTCores<DIM>());
  mapForChemicalPot["LIQUID"].emplace("U", TTCores<DIM>());
  mapForChemicalPot["LIQUID"].emplace("PU", TTCores<DIM>());
  ...
  for (size_t i = 0; i < chem_phases.size(); ++i) {
    ...
    for (const auto& elem : elements) {
      mapForChemicalPot[phase].emplace(elem, TTCores<DIM>());

      mapForChemicalPot[phase][elem].import_cores_from_hdf5(file, "mu_" + elem, coord_grid);
    }
  }
  // mapForChemicalMob["C1_MO2"].emplace("O", TTCores<DIM>());
  // mapForChemicalMob["C1_MO2"].emplace("U", TTCores<DIM>());
  // mapForChemicalMob["C1_MO2"].emplace("PU", TTCores<DIM>());
 ...
 */
template <typename T, std::size_t DIM>
void TT_Interpolation<T, DIM>::execute(
  ...
  Catch_Time_Section("TT_Interpolation::execute");

  std::string phase;
  if (status_phase.has_value()) {
    const auto& phase_vector = *status_phase;
    MFEM_VERIFY(phase_vector.size() == 1,
                "TT_Interpolation: status phase must contain exactly one element. \n");
    phase = std::get<0>(phase_vector[0]);
  } else {
    if (this->given_phase_.size() > 0) {
      phase = this->given_phase_;
    } else {
      throw std::runtime_error("TT_Interpolation::execute: GivenPhase parameter must be defined.");
    }
  }
...
template <typename T, std::size_t DIM>
void TT_Interpolation<T, DIM>::compute(
    const int dt, const std::set<int>& list_nodes, const std::vector<T>& tp_gf,
    const std::vector<std::tuple<std::string, std::string>>& chemical_system,
    const std::string& given_phase) {
  //         // TODOCP : refaire pour tenir compte des nouvelles fonctionnalités :)
  // Catch_Time_Section("TT_Interpolation::compute");
 ...
 */
template <typename T, std::size_t DIM>
void TT_Interpolation<T, DIM>::check_variables_consistency(
    std::vector<std::tuple<std::vector<std::string>, std::reference_wrapper<T>>>& output_system) {
  for (auto& [output_infos, output_value] : output_system) {
    ...
    switch (calphad_outputs::from(output_type)) {
      ...
      case calphad_outputs::hm: {
        MFEM_VERIFY(false, "TT_Interpolation is only built for mu, x and g.\n");
 ...
 */
template <typename T, std::size_t DIM>
void TT_Interpolation<T, DIM>::finalize() {
  fichier.close();
}
 ...
 */
template <typename T, std::size_t DIM>
TT_Interpolation<T, DIM>::~TT_Interpolation() {}
